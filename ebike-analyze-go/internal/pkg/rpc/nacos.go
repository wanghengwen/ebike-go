...
func registerNacosConfigListeners(dataID, group string) {
	nacosConfigListenOnce.Do(func() {
		...
		if err := config.ListenMySQLFromNacos(ConfigClient, func() {
			...
			// Reattach tenant Query callbacks on the new *gorm.DB handles.
...
func InitNacos() {
	...
	logDir, cacheDir := nacosRuntimeDirs()
	...
	// Redis: extension config redis.yaml in {group}_ops (same as Java bootstrap.yml).
	if err := config.ApplyRedisFromNacos(ConfigClient); err != nil {
		log.Printf("[WARN] %v", err)
	}

	// MySQL: extension config mysql.yaml in {group}_ops (same as Java bootstrap.yml).
...
// nacosRuntimeDirs returns writable log/cache directories for the Nacos SDK.
// On Windows, /tmp/nacos resolves to C:\tmp\nacos and often fails; use os.TempDir() instead.
func nacosRuntimeDirs() (logDir, cacheDir string) {
	if v := os.Getenv("NACOS_LOG_DIR"); v != "" {
		logDir = v
	} else {
		logDir = filepath.Join(os.TempDir(), "nacos", "log")
	}
	if v := os.Getenv("NACOS_CACHE_DIR"); v != "" {
		cacheDir = v
	} else {
