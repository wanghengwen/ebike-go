...
// VoiceHandler handles voice send requests.
...
// SendVoice handles POST /voice/send.
func (h *VoiceHandler) SendVoice(c *gin.Context) {
	var req model.SendVoiceReq
	if err := c.ShouldBindJSON(&req); err != nil {
		response.ErrorJSON(c, validator.FormatBindError(err, true))
		return
	}
	...
	if err := h.voiceService.SendVoice(ctx, &req); err != nil {
	...
	response.SuccessJSON(c, "发送语音任务提交成功，异步发送中")
}
