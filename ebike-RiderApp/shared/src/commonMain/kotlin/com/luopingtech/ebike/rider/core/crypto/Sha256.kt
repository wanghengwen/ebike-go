package com.luopingtech.ebike.rider.core.crypto

/**
 * Pure Kotlin SHA-256 (FIPS 180-4). Used for request signing on all targets.
 */
object Sha256 {
    fun hex(input: String): String = digest(input.encodeToByteArray()).toLowerHex()
    ...
    private fun pad(message: ByteArray): ByteArray {
        ...
        var newLen = message.size + 1
        while (newLen % 64 != 56) newLen++
        val padded = ByteArray(newLen + 8)
