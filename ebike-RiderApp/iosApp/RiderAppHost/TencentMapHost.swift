...
enum TencentMapHost {
    static func registerIfAvailable(config: TenantConfig) {
        ...
        guard !key.isEmpty else {
            NSLog("[RiderApp] Tencent map key empty; using MapKit.")
            return
        }
        #if RIDER_TENCENT_MAP
        let prefix = String(key.prefix(6))
        NSLog("[RiderApp] Registering QMapKit key \(prefix)… (\(key.count) chars)")
        TencentMapFactory.register(apiKey: key)
        #else
        NSLog("[RiderApp] Tencent map SDK not enabled for this destination (simulator); using MapKit. Key present (\(key.count) chars).")
        #endif
    }
}
