...
fun FreeMoveCarSection(
    ...
    val takePictureLauncher = rememberLauncherForActivityResult(
        ...
        if (ok && uri != null) {
            app.freeMoveCarFeature.addPhotoUrl(uri.toString())
            photoHint = null
        } else {
    ...
    val cameraPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (granted) {
            val uri = createCachePhotoUri(context, prefix = "freemove")
    ...
    fun launchCamera() {
        ...
        if (granted) {
            val uri = createCachePhotoUri(context, prefix = "freemove") ?: return
            pendingCameraUri = uri
            takePictureLauncher.launch(uri)
        } else {
    ...
    if (state.selectedCarIds.isNotEmpty() || state.needPhotograph || state.photoUrls.isNotEmpty()) {
        ...
        Row(
            ...
            Button(onClick = { launchCamera() }, modifier = Modifier.weight(1f)) { Text(t(Str.Camera)) }
            Button(
