...
/**
 * Continuous location for field-ops track upload.
 * Hosts request runtime permission before [startTracking] on real devices.
 */
interface LocationTracker {
    suspend fun currentLocation(): OpsResult<GeoPoint>
...
class UnsupportedLocationTracker : LocationTracker {
    override suspend fun currentLocation(): OpsResult<GeoPoint> =
        OpsResult.Err(OpsError.unsupported("LocationTracker"))

    override fun track(): Flow<GeoPoint> = flowOf()
}

interface CodeScanner {
    suspend fun scanOnce(): OpsResult<String>
}

class UnsupportedCodeScanner : CodeScanner {
    override suspend fun scanOnce(): OpsResult<String> =
        OpsResult.Err(OpsError.unsupported("CodeScanner"))
}
...
class BindableCodeScanner : CodeScanner {
    ...
    override suspend fun scanOnce(): OpsResult<String> {
        ...
        return current?.scanOnce()
            ?: OpsResult.Err(OpsError.unsupported("CodeScanner not bound to Activity"))
    }
}
...
interface PhotoCapture {
    ...
    suspend fun takePhoto(prefix: String = "photo"): OpsResult<String>

    suspend fun pickFromGallery(): OpsResult<String>
}

class UnsupportedPhotoCapture : PhotoCapture {
    override suspend fun takePhoto(prefix: String): OpsResult<String> =
        OpsResult.Err(OpsError.unsupported("PhotoCapture"))

    override suspend fun pickFromGallery(): OpsResult<String> =
        OpsResult.Err(OpsError.unsupported("PhotoCapture"))
}
...
class BindablePhotoCapture : PhotoCapture {
    ...
    override suspend fun takePhoto(prefix: String): OpsResult<String> =
        bound?.takePhoto(prefix)
            ?: OpsResult.Err(OpsError.unsupported("PhotoCapture not bound to Activity"))
...
object MapCapabilityFactory {
    fun fromConfig(config: com.luopingtech.ebike.ops.core.config.TenantConfig): MapCapability {
        ...
        return when (raw) {
            ...
            "tencent" -> {
                if (config.map.tencentKey.isNotBlank()) {
                    ...
                    // No key → simulator so field-ops UI remains usable.
...
interface MediaUploader {
    suspend fun upload(localUris: List<String>): OpsResult<List<String>>
}
            ...
            localUris.map { uri ->
                ...
                when {
                    ...
                    trimmed.startsWith("demo://") ->
                        "https://demo.cdn.ops/${trimmed.removePrefix("demo://")}"
                    else ->
                        "https://demo.cdn.ops/upload/${trimmed.hashCode().toUInt()}.jpg"
                }
            },
        )
    }
...
class BindableMediaUploader : MediaUploader {
    ...
    override suspend fun upload(localUris: List<String>): OpsResult<List<String>> {
        ...
        return current?.upload(localUris)
            ?: OpsResult.Err(OpsError.unsupported("MediaUploader not bound"))
    }
}

interface PushRegistrar {
    val isEnabled: Boolean
    suspend fun register(): OpsResult<String>
}

class DisabledPushRegistrar : PushRegistrar {
    override val isEnabled: Boolean = false
    override suspend fun register(): OpsResult<String> =
        OpsResult.Err(OpsError.unsupported("PushRegistrar"))
}
