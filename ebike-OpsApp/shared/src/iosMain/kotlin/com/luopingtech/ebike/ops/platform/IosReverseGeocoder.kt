    ...
    override suspend fun addressOf(lat: Double, lng: Double): OpsResult<String> =
        suspendCancellableCoroutine { cont ->
            ...
            geocoder.reverseGeocodeLocation(location) { placemarks, error ->
                ...
                if (error != null) {
                    cont.resume(OpsResult.Err(OpsError.network(error.localizedDescription)))
                    return@reverseGeocodeLocation
                }
                val mark = placemarks?.firstOrNull() as? CLPlacemark
                val line = mark?.let(::describe).orEmpty()
                if (line.isBlank()) {
                    cont.resume(OpsResult.Err(OpsError.business("GEO", "empty address")))
                } else {
                    cont.resume(OpsResult.Ok(line))
                }
            }
