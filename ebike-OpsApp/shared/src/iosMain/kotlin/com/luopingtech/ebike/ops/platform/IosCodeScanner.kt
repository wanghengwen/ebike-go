 ...
 *
 * 界面里的取景框走 `OpsScanPreview`，这里是给 feature 层用的：全屏模态 + 一个取消按钮，
        ...
        return suspendCancellableCoroutine { cont ->
            onMain {
                ...
                if (!scanSession.configure()) {
                    cont.resume(OpsResult.Err(OpsError.unsupported("Camera unavailable")))
                    return@onMain
                }
                var settled = false
                val controller = ScanModalController(scanSession, cancelTitle) { code ->
                    ...
                    settled = true
                    cont.resume(
                        code?.let { OpsResult.Ok(it) }
                            ?: OpsResult.Err(OpsError.business("SCAN_CANCELLED", cancelTitle)),
                    )
                }
