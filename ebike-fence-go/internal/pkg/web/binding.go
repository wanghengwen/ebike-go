// Package web provides request-binding and response helpers, mirroring
// ebike-fence GlobalExceptionHandler behavior:
//
...
func writeBindError(c *gin.Context, err error, msgs map[string]string, jsonBody bool) {
	...
	if jsonBody {
		...
		c.JSON(http.StatusOK, dto.NewErrorResult(dto.CodeParamException, "http message not readable 2:"))
		return
	}
...
// WriteBizError mirrors Java BizException: success=false with business code/msg.
func WriteBizError(c *gin.Context, code, msg string) {
	c.JSON(http.StatusOK, dto.Result{Success: false, Code: &code, Msg: &msg})
}
...
// numeric zero and bypass null checks, which diverges from Java @NotNull Long.
