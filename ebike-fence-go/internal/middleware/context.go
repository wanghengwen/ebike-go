...
// Mirrors Java CommandContextAspect: Assert.notNull(commandContext/traceId/tenantId).
func ApplyCommand(c *gin.Context, cmd *dto.Command) (*dto.CommandContext, bool) {
	...
	if ctx.TenantId == "" {
		writeCommandContextError(c, "tenantId 不能为null")
		return nil, false
	}
...
func writeCommandContextError(c *gin.Context, msg string) {
	c.JSON(http.StatusOK, dto.NewErrorResult(dto.CodeNotNull, msg))
	c.Abort()
}
